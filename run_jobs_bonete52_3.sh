...
python -m pipeline train_sft \
  ...
  --base-model artifacts/competition_math/models/baseline_models/qwen3-4b-base/model \
...
python -m pipeline train_rl \
  ...
  --method method_ac \
  --run-id qwen3-4b-base \
  --sft-model artifacts/competition_math/models/method_ac_sft/qwen3-4b-base/model \
