...
python -m pipeline train_sft \
  ...
  --method method_ac \
  --run-id qwen2.5-3b \
  --base-model artifacts/competition_math/models/baseline_models/qwen2.5-3b/model \
...
python -m pipeline train_rl \
  ...
  --method method_ac \
  --run-id qwen2.5-3b \
  --sft-model artifacts/competition_math/models/method_ac_sft/qwen2.5-3b/model \
